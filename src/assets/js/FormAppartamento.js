...
$(".next").click(function() {
  ...
  $(this).parent().find('[required]').each(function() {
    if ($(this).is('img') && $(this).attr('src') == "https://icon-library.com/images/add-picture-icon/add-picture-icon-16.jpg") {
      valid = false;
      $(this).addClass('is-invalid');
    } else if (!$(this).is('img') && ($(this).is(':invalid') || !$(this).val() || indirizzo.via == "")) {
      valid = false;
      $(this).addClass('is-invalid');
    }
  })
  if (!valid) {
    document.getElementById('msform').scrollIntoView({behavior: "smooth", block: "end"})
    return;
  }
  ...
  if (next_fs.attr('id') == 'latest') {
    var appartamento = {
      user_id: 0,
      email: "name@mail.com",
      titolo: $("#tipologiaAnnuncio option:selected").text() + " - " + indirizzo.via + ", " + indirizzo.citta,
...
$(".add").click(function() {
  ...
  $(this).closest(".row").find(".add").before('<div class="col-sm-3"><img class="img-thumbnail" src="https://icon-library.com/images/add-picture-icon/add-picture-icon-16.jpg"><i class="fa fa-xmark del"></i><input type="file" accept="image/*" class="uploadFile" value="Carica Foto" style="width:0px;height:0px;overflow:hidden;"></div>');
});
...
$(document).on("click", "i.del", function() {
  ...
  if (value != "https://icon-library.com/images/add-picture-icon/add-picture-icon-16.jpg") {
...
$(document).on("change", ".uploadFile", function() {
  ...
  if (/^image/.test(files[0].type)) {
      ...
      reader.onloadend = function() {
        uploadFile.closest(".col-sm-3").find('.img-thumbnail').attr('src', this.result);
